...
float Processor::Utilization() {
  ...
  float total_diff = static_cast<float>(total_new - total_old);
  float idle_diff = static_cast<float>(idle_new - idle_old);
  // Current CPU memory usage
  return (total_diff - idle_diff) / total_diff;
  //return static_cast<float>(CurrentActive()/CurrentTotal());
}
